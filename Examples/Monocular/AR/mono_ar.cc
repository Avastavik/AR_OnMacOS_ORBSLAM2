...
{
public:
    SLAMRunner(ORB_SLAM2::System* pSLAM):mpSLAM(pSLAM){}
...
int main(int argc, char **argv)
{
    ...
    if(argc != 3)
    {
        cerr << endl << "Usage: mono_ar path_to_vocabulary path_to_settings" << endl;
        return 1;
    }
    cout<< "args[0]" << argv[0];
    cout<<"args[1]" << argv[1];
    ...
    thread tSlamRunner = thread(&SLAMRunner::run,&runner);
...
void SLAMRunner::run()
{
  ...
  while(1){
    cap >> image;
    imwrite("stillimage.jpg", image);

    auto t2 = std::chrono::steady_clock::now().time_since_epoch();
    double ttrack = std::chrono::duration_cast<std::chrono::duration<double> >(t2).count();

    cv::Mat Tcw = mpSLAM->TrackMonocular(image,ttrack);
    int state = mpSLAM->GetTrackingState();
    vector<ORB_SLAM2::MapPoint*> vMPs = mpSLAM->GetTrackedMapPoints();
    vector<cv::KeyPoint> vKeys = mpSLAM->GetTrackedKeyPointsUn();

    cv::Mat imu;
    cv::undistort(image,imu,K,DistCoef);
